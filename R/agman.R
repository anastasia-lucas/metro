...
#' @export
#' @examples
#' agman(d, format, line, log10, yaxis, title, chrcolor1, chrcolor2, groupcolors, file, hgt, wi, res)

agman <- function(d, format="plotman", line, log10=TRUE, yaxis, highlight_snp, highlight_p, highlighter="red", title=NULL, chrcolor1="#AAAAAA", chrcolor2="#4D4D4D", groupcolors, file="agman", hgt=7, wi=12, res=300 ){
  ...
  if("Shape" %in% names(d)){
    p <- p + geom_point(data=d_order, aes(x=pos_index, y=pval, color=factor(Color), shape=factor(Shape), frame=Frame))
  } else {
    p <- p + geom_point(data=d_order, aes(x=pos_index, y=pval, color=factor(Color), frame=Frame))
  }
  ...
  ap <- gganimate(p)
  gganimate_save(ap, filename=paste(file, ".html", sep=""))
  return(ap)
}
